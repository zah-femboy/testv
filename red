local CFGFolder = "ZXCNIGGER"
local CheatName = 'Seere<font color="#a0a1e9">.vip</font> - Mega OMG Fix😳'
local ColorLines = Color3.fromRGB(160, 161, 233)--208, 123, 255
--UI
...
local library = {
	...
	flags = {};
	scrolling = true;
	notifyLine = Drawing.new("Line");
	notifyBox = Drawing.new("Square");
	notifyBoxOutline = Drawing.new("Square");
...
function draggable(a)
	...
	end)a.InputChanged:Connect(function(h)if h.UserInputType==Enum.UserInputType.MouseMovement or h.UserInputType==Enum.UserInputType.Touch then d=h end end)b.InputChanged:Connect(function(h)if h==d and c then g(h)end end)end
  draggable(menu.bg)

  local MenuBind = Enum.KeyCode.Delete
  ...
  local Menu = {cursor = true,mouseicon = false,colour = Color3.fromRGB(208, 123, 255),outline = Color3.fromRGB(0, 0, 0)}

  local Cursor = Drawing.new('Triangle') Cursor.Thickness = 1 Cursor.Filled = true;local CursorOutline = Drawing.new('Triangle') CursorOutline.Thickness = 1 CursorOutline.Filled = false
  inputService.InputBegan:Connect(function(input)
  if input.KeyCode == MenuBind then Menu.cursor = not Menu.cursor Cursor.Visible = Menu.cursor CursorOutline.Visible = Menu.cursor end
  end)
  inputService.MouseIconEnabled = false inputService.InputBegan:Connect(function(input)
  if input.KeyCode == MenuBind then Menu.mouseicon = not Menu.mouseicon inputService.MouseIconEnabled = Menu.mouseicon inputService.MouseIconEnabled = Menu.mouseicon end
  end)

  runService.RenderStepped:Connect(function()
  if Menu.cursor then
    Cursor.PointA = Vector2.new(inputService:GetMouseLocation().X, inputService:GetMouseLocation().Y);Cursor.PointB = Vector2.new(inputService:GetMouseLocation().X + 16, inputService:GetMouseLocation().Y + 6);Cursor.PointC = Vector2.new(inputService:GetMouseLocation().X + 6, inputService:GetMouseLocation().Y + 16)
    CursorOutline.Visible = Menu.cursor;CursorOutline.PointA = Cursor.PointA;CursorOutline.PointB = Cursor.PointB;CursorOutline.PointC = Cursor.PointC;Cursor.Visible = Menu.cursor;Cursor.Color = library.libColor;CursorOutline.Color = Menu.outline
  end
  end)

  local keyNames = {[Enum.KeyCode.LeftAlt] = 'LALT';[Enum.KeyCode.RightAlt] = 'RALT';[Enum.KeyCode.LeftControl] = 'LCTRL';[Enum.KeyCode.RightControl] = 'RCTRL';[Enum.KeyCode.LeftShift] = 'LSHIFT';[Enum.KeyCode.RightShift] = 'RSHIFT';[Enum.KeyCode.Underscore] = '_';[Enum.KeyCode.Minus] = '-';[Enum.KeyCode.Plus] = '+';[Enum.KeyCode.Period] = '.';[Enum.KeyCode.Slash] = '/';[Enum.KeyCode.BackSlash] = '\\';[Enum.KeyCode.Question] = '?';[Enum.UserInputType.MouseButton1] = 'MB1';[Enum.UserInputType.MouseButton2] = 'MB2';[Enum.UserInputType.MouseButton3] = 'MB3';}
  function library:Tween(...)
    tweenService:Create(...):Play()
  end

  library.notifyLine.From = Vector2.new(10, 60);library.notifyLine.To = Vector2.new(10, 80);library.notifyLine.Color = Color3.fromRGB(208, 123, 255);library.notifyLine.Thickness = 6
  library.notifyBox.Position = Vector2.new(10,60);library.notifyBox.Size = Vector2.new(300, 20);library.notifyBox.Color = Color3.fromRGB(20, 20, 20);library.notifyBox.Filled = true
  library.notifyBoxOutline.Position = Vector2.new(10,60);library.notifyBoxOutline.Size = Vector2.new(300, 21);library.notifyBoxOutline.Color = Color3.fromRGB(50, 50, 50);library.notifyBoxOutline.Filled = false
  library.notifyText.Font = 2;library.notifyText.Size = 14;library.notifyText.Outline = true;library.notifyText.Color = Color3.new(1,1,1);library.notifyText.Position = Vector2.new(15,62)

  function library:notify(text,time,color)
    if playing then return end
    playing = true
    library.notifyText.Text = text
    library.notifyLine.Visible = true;library.notifyBox.Visible = true;library.notifyText.Visible = true;library.notifyBoxOutline.Visible = true;library.notifyBox.Transparency = 0;library.notifyBoxOutline.Transparency = 0;library.notifyText.Transparency = 0;library.notifyLine.Transparency = 0
    for i = 0,1,0.1 do wait()
    library.notifyLine.Transparency = i;library.notifyBox.Transparency = i;library.notifyBoxOutline.Transparency = i;library.notifyText.Transparency = i
  end
  spawn(function()
  wait(time)
...
playing = false
library.notifyLine.Visible = false;library.notifyBox.Visible = false;library.notifyBoxOutline.Visible = false;library.notifyText.Visible = false
end)
end
...
function tab:createGroup(pos,groupname)
  ...
  element3.Color = ColorSequence.new{ColorSequenceKeypoint.new(0.00, library.libColor), ColorSequenceKeypoint.new(1.00, Color3.fromRGB(0, 0, 0))}
  ...
  function group:addToggle(args)
    ...
    library.options[args.flag] = {type = "toggle",changeState = toggle,skipflag = args.skipflag,oldargs = args,toggle = state}
    local toggle = {}
    ...
    function toggle:addColorpicker(args)
      ...
      copy.TextColor3 = library.libColor
  ...
  function group:addList(args)
    ...
    local function updateValue(value)
      ...
      if args.multiselect then
        ...
        for i,v in next, holder:GetChildren() do
          ...
          v.off.TextColor3 = Color3.new(1,0.65,0.65)
    ...
    function refresh(tbl)
      ...
      for i,v in pairs(tbl) do
        ...
        text_2.Text = v
        text_2.TextColor3 = args.multiselect and Color3.new(0.65,0.65,0.65) or library.libColor
        ...
        button_2.BorderColor3 = library.libColor
...
for _, v in pairs(list) do
  ...
  writefile(CFGFolder .. "\\"..name..".cfg",game:GetService("HttpService"):JSONEncode(jig))
...
function library:saveConfig()
  ...
  function library:loadConfig()
    ...
    if not isfile(name) then
      ...
      elseif v.type == "slider" then v.changeState(v.args.value or 0)
...
function library:refreshConfigs()
  local tbl = {}
  for i,v in next, listfiles(CFGFolder) do table.insert(tbl,v) end
